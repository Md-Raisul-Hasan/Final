    ...
    if (isset($_POST["annotatedtext"])) {
        if (isset($_POST["Level"])) {
            ...
            if ($Protected_query->execute()) {
                ...
                $annotationQuery = "INSERT INTO `Scribii`.Annotation(Essay_input_id, Annotation_id, Annotated_text, Sub_skill, Sub_skill_rgb_color, Comment) VALUES(?, ?, ?, ?, ?, ?)";
                if ($annotationStmt = $conn->prepare($annotationQuery)) {
                    foreach ($annotatedSentences as $index => $sentence) {
                        $sub_skill = $ss_col[$colors[$index]];
                        $AnnoINPUTID = $prefix_AI . ($last_AII_id + $index + 1);
                        $annotationStmt->bind_param("ssssss", $EssayINPUTID, $AnnoINPUTID, $sentence, $sub_skill, $colors[$index], $comment);
            ...
            echo "Error: POST parameter 'level' is missing.";
